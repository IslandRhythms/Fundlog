...
- **Local database**: Uses `better-sqlite3` for a local, file-based database (no external DB server required). Optional custom database path and export from **Settings → Data & sync**. Custom paths must be chosen via the native file dialog (not typed freely). Foreign keys are enforced (`PRAGMA foreign_keys = ON`). **Backup reminders**: set “Remind every N days” in Settings (0 = off); after a successful export the app records the time, and when overdue a dismissible banner offers one-click export.
...
- **Transactions**: The **Transactions** page loads data in pages (~50 rows) with server-side search (merchant/description), date range, and subcategory filters. Dashboards and related views load **current-month** transaction sets where possible instead of the full history.
...
- **Database location**: The app uses `better-sqlite3` to access a local SQLite database file; defaults and overrides are described in **Settings** and in the main-process DB helpers. Schema migrations (including tables for goals, extra monthly income lines, portfolio accounts/snapshots, query indexes, etc.) run automatically on startup via `main-process/db.ts`. Foreign keys are enabled on every open.
